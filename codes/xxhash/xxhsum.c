...
CAMLprim value xx32sum_main(value ml_array)
{
    CAMLparam1( ml_array );
    int i, filenamesStart=0, argc;
    const char** argv = ml_array;
    argc = Wosize_val(ml_array);
    ...
    for(i=1; i< argc; i++)
    {
        const char* argument = String_val(Field(ml_array,i));
        ...
        while (*argument!=0)
        {
            ...
            case 'h':
                return Val_int(usage_advanced(exename));
            ...
            default:
                return Val_int(badusage(exename));
            }
        }
    }
    ...
    if (benchmarkMode)
    {
        ...
        return Val_int(BMK_benchFiles(argv+filenamesStart, argc-filenamesStart));
    }
